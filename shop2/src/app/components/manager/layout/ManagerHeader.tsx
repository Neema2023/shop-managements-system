...
import { Menu, X, Bell, AlertCircle, LogOut } from 'lucide-react';
      ...
      <header className="bg-white border-b border-gray-200 px-4 md:px-6 py-3 md:py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div>
              <h1 className="text-lg md:text-2xl font-bold text-gray-800">Manager Dashboard</h1>
              <p className="text-xs md:text-sm text-gray-500">Inventory & Staff Management</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div className="hidden md:flex items-center gap-3">
              <div className="text-right">
                <p className="text-sm font-medium text-gray-800">{userData?.name || 'Manager User'}</p>
                <p className="text-xs text-gray-500">Store Manager</p>
              </div>
              <div className="w-8 h-8 md:w-10 md:h-10 bg-gradient-to-br from-teal-400 to-cyan-500 rounded-full flex items-center justify-center text-white font-semibold text-sm md:text-base">
                {userData?.name?.split(' ').map((n: string) => n[0]).join('').toUpperCase() || 'MU'}
              </div>
            </div>
